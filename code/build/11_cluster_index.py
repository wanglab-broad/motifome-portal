...
OUT_JS = paths.REPO / "portal" / "js" / "cluster-index.js"
...
def main():
    ...
    OUT_JS.write_text(
        "/* GENERATED by code/build/11_cluster_index.py — do not edit by hand.\n"
        "   Same bytes as portal/data/cluster_index.json.  Imported (not fetched) by\n"
        "   js/views/browse.js and js/views/cluster.js because data.js owns fetching\n"
        "   and has no getter for this payload yet. */\n"
        "export default " + blob + ";\n")

    c = meta["counts"]
    print(f"[cluster_index] {len(rows)} rows -> {OUT_JSON.name} "
          f"({len(blob):,} B) and js/cluster-index.js")
